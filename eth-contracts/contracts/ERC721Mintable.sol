...
contract Ownable {
    ...
    constructor () internal {
        _owner = msg.sender;
        emit OwnershipTransfer(msg.sender);
    } 
    //  3) create an 'onlyOwner' modifier that throws if called by any account other than the owner.
    modifier onlyOwner() {
        require(msg.sender == _owner, "Only owner is allwoed to do this action");
        _;
    }
...
contract Pausable is Ownable {
    ...
    function changeStatus(bool paused) public onlyOwner {
        require(paused != _paused, "Contract already in that state");
        _paused = paused;
        if(_paused) {
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(currentOwner != to, "Given address already owns selected token");
        // TODO require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require(msg.sender == currentOwner || isApprovedForAll(currentOwner, msg.sender), "Unable to approve");
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(ownerOf(tokenId) == from, "Can not transfer unowned tokens");
        // TODO: require token is being transfered to valid address
        require(to == address(to), "Invalid transfer address");
        // TODO: clear approval
...
contract ERC721Metadata is ERC721Enumerable, usingOraclize {
    ...
    function setTokenURI(uint256 tokenId) internal {
        ...
        //_tokenURI[tokenId] = string(abi.encodePacked(uintToString(tokenId)), _baseTokenURI);
...
contract RealEstateContractERC721 is ERC721Metadata(
    "IllegalRealEstates",
    "IRE",
